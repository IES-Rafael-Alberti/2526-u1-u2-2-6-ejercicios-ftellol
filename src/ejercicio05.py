...
def convertir_segundos(segundos_totales: int) -> tuple[int, int, int, int]:
    ...
    horas = 0
    minutos = 0
    segundos_sobrantes = segundos_totales

    if segundos_totales < 0:
        dias = 0
        horas = 0
        minutos = 0
        segundos_sobrantes = 0

    if segundos_totales >= 86400:
        dias = segundos_totales // 86400
        segundos_sobrantes = segundos_totales % 86400

    if segundos_sobrantes >= 3600:
        horas = segundos_sobrantes // 3600
        segundos_sobrantes = segundos_sobrantes % 3600

    if segundos_sobrantes >= 60:
        minutos = segundos_sobrantes // 60
        segundos_sobrantes = segundos_sobrantes % 60

    segundos = segundos_sobrantes
